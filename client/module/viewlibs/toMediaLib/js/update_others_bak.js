	...
	var OthersForm = new new Class({
		...
		initialize: function() {
			...
			jQuery("#otherForm").validate({
				...
				submitHandler: function() {
					if (jQuery("#otherForm").find(".error").length === 0) {
						jQuery("#otherForm")[0].submit();
					} else {
						return;
					}
				}
		...
		bindEvents: function() {
			...
			jQuery(document).on('click', "form .input-submit", function(e) {
				e.preventDefault();
				var formdata = self.serializeForm(jQuery("#otherForm").serializeArray()),
				...
				formdata = JSON.stringify(Object.merge(formdata, {
					"remark": setRemark.getText(jQuery("#otherForm")),
				...
				if (jQuery("#otherForm").valid() && jQuery("#otherForm").find(".error").length === 0 && jQuery('.notNull').find("option:selected").val() !== "") {
					jQuery.ajax({
						url: url,
						data: {
							"formData": formdata
						...
						success: function(data) {
							if (data && data.code === 200) {
								...
								var type = (parseInt(parent.window.opener.gMessJson.fileType, 10) === 2 ? '图片' : '视频'),
									...
									str = incident ? '案事件中' : '';
								logDict.insertMedialog('m4', incident + str + name + type + '生成其他结构化信息');
								...
								// var data = window.opener.gMessJson.cloudId + "," + window.opener.gMessJson.mediaPath + "," + window.opener.gMessJson.shootTime + "," + window.opener.gMessJson.fileType + "," + window.opener.gMessJson.medialibId + ",6,"; //结构化类型 其他是6
