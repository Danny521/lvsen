...
define([
	...
	var CommonOnePlayer = new Class({
		...
		options: {
			...
			enableDragMark: false, // 是否可拖动标记按钮
			markTime: { // 时间片段
				startTime: 0,
				endTime: 0
			},
		...
		time2px: function(time) {
			//alert(time+'--'+this.allTime+'---'+this.progress_bar.width());
			var a= Math.min(Math.max((time / this.allTime) * this.progress_bar.width(), 0), this.progress_bar.width());
			//alert(a)
			return a;
		},
		px2time: function(px) {
		...
		play: function() {
			...
			if (result) {
				this.playStatus = 1;
				this.btn_play.addClass('active').attr("title", "暂停");
				...
				if (isStop) {
					this.options.enableCutMarkTime = true;
					this.setCutMarkTime();
				};
			};
			return result;
		...
		setPlaySpeed: function(speed) {
			if (this.play()) {
				...
				// 获取播放速度
				var result = this.getPlaySpeed();

				// 更新播放速度显示
				if (result === "1") {
					this.time_speed.hide();
				} else {
					this.time_speed.show().find("span").text(result + "x");
				};
			};
		},
