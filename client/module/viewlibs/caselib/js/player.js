define([
    '/module/common/js/base-player.js',
    'base.self'
    ...
    var Mplayer = new new Class({
        ...
        initPlayer: function(playParm, callback) {
            var self = this;
            ...
            // 获取时间范围
            if (window.opener && window.opener.gMessJson) {
        ...
        getVersion: function() {
            var self = this;
            var version = self.player.getVersion();
            return version;
        },
        ...
        togglePlaystatus: function(){
            ...
            jQuery(".video-block .switch").removeClass("active");
            jQuery(".video-block .switch").addClass("over");
        },
        bindEvents: function() {
            var self = this;
            jQuery(function() {
                        ...
                        if (jQuery(this).is(".active")) {
                            ...
                            if (jQuery(this).hasClass("over")) { //关闭后的播放
                                self.player.playPfs2(self.playParm,0,function(){
                                     if (self.playRange) {
                                        self.player.playFormStartToEnd2(parseInt(self.timeBegin), parseInt(self.timeEnd), false, 0,self.togglePlaystatus,1);
                                        jQuery(".video .video-block .speed").hide();
                                    };
                                })
                               /* if (self.player.playPfs(self.playParm, 0)) {
                                    ...
                                    if (self.playRange) {
                                        self.player.playFormStartToEnd2(parseInt(self.timeBegin), parseInt(self.timeEnd), false, 0,self.togglePlaystatus,1);
                                        //self.player.playFormStartToEnd(self.timeBegin, self.timeEnd, false, 0);
                                    };
                                }*/
