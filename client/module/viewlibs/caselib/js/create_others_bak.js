	...
	var OthersForm = new new Class({
		...
		bindEvents: function() {
			...
			jQuery(document).on('click', "form .input-submit", function(e) {
				e.preventDefault();
				var formdata = self.serializeForm(jQuery("#othersForm").serializeArray()),
					id = parent.window.opener.gMessJson.medialibId,
					url = "/service/pvd/rest" + "?timestamp=" + new Date().getTime();
				var videoId = "",
					imageId = "";
				...
				formdata = JSON.stringify(Object.merge(formdata, {
					...
					"imageId": imageId,
					"sourceId": id,
					"remark": setRemark.getText(jQuery("#othersForm")),
				...
				if (jQuery("#othersForm").valid() && jQuery("#othersForm").find(".error").length === 0 && jQuery('.notNull').find("option:selected").val() !== "") {
					//保存按钮置灰
						...
						success: function(data) {
							...
							if (data && data.code === 200) {
								...
								if (parent.window.opener.gMessJson.source && parent.window.opener.gMessJson.source === "unviewlib") {
									...
									dialogString = dialogString + "<a href='/module/iframe/?windowOpen=1&iframeUrl=/module/viewlibs/details/struct/others.html?origntype=others", orgid = "";
