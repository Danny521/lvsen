 ...
 */
 define(['basePlayer','base.self'],function(NativePlayer){
	var CommonOnePlayer = new Class({
		...
		options: {
			container: null,
			tplUrl: '/module/common/addPlayer/new-player.html',
			fileUrl: "",
			// 人工标注时用到这三个属性
		...
		createElements: function() {
			...
			self.player = new NativePlayer({
				layout: 1,
				uiocx: self.options.container + " .player-obj"
			});
		},
		bindEvents: function() {
			var self = this;
			...
			self.btn_screenshot.on("click", function(event) {
				...
				if (data === "ERROR") {
					...
					self.fireEvent("screenshot", data);
				}
			});
		...
		playFirstFrame: function() {
			var self = this;
			// 播放视频第一帧
			var result = self.player.playPfs2({
			...
			if (this.options.enableCutMarkTime && this.options.cutMarkTime.length > 0) {
				if (this.tempCacheMarkInfo) {
					if (this.timeInRange(this.nowTime, this.tempCacheMarkInfo.startTime, this.tempCacheMarkInfo.endTime)) {
						var $speedMark = jQuery('.speed-mark'),
							$speedNomark = jQuery('.speed-nomark');
						if ($speedMark.length && this.options.cutMarkSpeed !== $speedMark.val()) {
							this.options.cutMarkSpeed = $speedMark.val();
							this.playBySpeed(this.options.cutMarkSpeed);
						}

						if ($speedNomark.length && this.options.cutNoMarkSpeed !== $speedNomark.val()) {
							this.options.cutNoMarkSpeed = $speedNomark.val();
		...
		playByRange: function(startTime, endTime) {
			if (this.play()) {
				this.player.playFormStartToEnd(startTime, endTime, true, 0);
			};
		},
		...
		updatePlayTime: function(interval) {
			...
			window.clearTimeout(self.updatePlayTimeTimeoutId);
			self.updatePlayTimeTimeoutId = setTimeout(function() {
				self.updatePlayTime(interval);
			}, interval);
		},
		clearPlayTime:function(){
			var self = this;
			return window.clearTimeout(self.updatePlayTimeTimeoutId );
		},
		...
		setPlaySpeed: function(speed) {
			if (this.play()) {
				...
				var oldSpeed = this.player.getPlaySpeed();

				// 设置播放速度
				this.player.setPlaySpeed(speed, 0);

				// 获取播放速度
				var newSpeed = this.getPlaySpeed();
				if (newSpeed === "1/16") {
					notify.warn("已达到最小倍速播放。");
					return;
				}
				if (parseInt(newSpeed) === 16) {
				...
				if (newSpeed === "1") {
					...
					this.time_speed.show().find("span").text(newSpeed + "x");
				};

				oldSpeed = eval(oldSpeed);
				newSpeed1 = eval(newSpeed);
				if (newSpeed1 !== newSpeed) {
					this.fireEvent("speedChange", [this.inTimeRange, newSpeed]);
				};
			};
		},
