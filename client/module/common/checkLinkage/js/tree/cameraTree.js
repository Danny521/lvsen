...
/**
 * Created by Zhangyu on 2014/10/6.
 * 防控规则设置部分的左侧资源树
 */
define(['jquery', 'mootools'], function(jQuery) {
	var AlarmCamera = new Class({
		...
		options: {
			...
			templateUrl: "/module/common/checkLinkage/inc/cameraTree.html",
			...
			scrollbar: null,
			type: "org",
			scrollbarNode: "#defence-setting-aside .scrollbarPanel",
			selectable: false,
			defaultRootId: 0,
			orgId: null,
		...
		reload: function (options) {
			var tem = this.options;
			this.getOrgPathList(this.options.orgId);
		...
		search: function (options) {
			...
			jQuery(tem.node).html("");
			if (tem.queryKey !== "") {
				this.loadData({"key": tem.queryKey, "mark": tem.isMarked, "type": "org", "count": 50, "offset": 0, "sign": options.linkageType}, jQuery(tem.node), false);
			} else {
				this.reload();
			}
		},
		...
		addHelper: function () {
			Handlebars.registerHelper('isGroup', function (type, options) {
				if (type === "group") {
			...
			Handlebars.registerHelper("isHidden", function(type) {
				var linkageType = jQuery(".popLeft li.active").data("tab");
				if (linkageType === "ptz") {
					if (type === "0" || type === 0) {
						return "hidden";
					}
		...
		bindEvent: function (parentNode, initFlag) {
			...
			// 叶子节点前的单选按钮图片点击事件
			parentNode.find("ul li.leaf i.radio-leaf").on("click", function (event) {
				// 直接调用span的点击事件
				self.processLeafClick(jQuery(this).closest("li").children("span"));
				return false;
			});
		...
		loadData: function (params, parentNode, initFlag) {
			...
			var self = this, url = self.options.url + "?type=" + self.options.type + "&isRoot=1", getRootFlag = false, requestType = "get";
					...
					if (getRootFlag) {
						...
						if (res && res.code === 200 && res.data.cameras.length > 0) {
							...
							if (params.key) {
								var mine = [];
								if (params.sign === "ptz") {
									mine = receiveData.filter(function(item) {
										return item.camera_type === 1;
									});
								} else {
									mine = receiveData;
								}
								...
								self.appendHTML(mine.splice(0, 49), parentNode, self, initFlag);
