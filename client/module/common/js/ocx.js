...
/**
 * [addEvent 绑定事件的选择，这个区分是否支持attachEvent和addEventListener
 * 来做判断的，但是在ie11下也支持addEventListener,但是ocx对于ie的事件绑定只
 * 对ie的绑定方式有效，如果写成注释掉的代码的话就会造成ie11下事件绑定不上，所
 * 以采用第二种方式让优先判断attachEvent]
 * @author Wang Xiaojun
 * @date   2014-12-05
 */
...
var VideoPlayer = new Class({
	...
	options: {
		...
		resize:false,
		eventEnable:true,
		uiocx: 'UIOCX',//以后给uiocx赋值的时候是id但不用带#，因为采用jQuery是找不到元素，只有用原生的可以 by wangxiaojun (高版本的是可以的)
	...
	**/
	playStream:function(options, index){
		...
		options.type = 1;
		var jsonstr = JSON.stringify(options);//console.log("stime:",(new Date()).getTime());
	...
	getOnlineChannels:function(channels){
		...
		if(typeOf(channels)==='array'){
			channels.each(function(item,index){
				 ...
				 */
				if(item.channel_status===0 || window.offlineCameraPlay){
    ...
    setLayoutBySH:function(layout){
        ...
        if(layout===1){
            ...
            if(this.cameraData[0]!==-1){
            	//有高清通道才切换成高清
            	if(this.cameraData[0].hdChannel.length>0) this.switchDefinition(0,1);
            }
        }else{
	...
	setDisplayStyle:function(option,index){
		...
		switch(status)
		{
			case 0://正常
				...
				//球击
				if(type===1 && window.LoopInspect&&!window.LoopInspect.isGoing){
	...
	setPlayerStyle:function(index){
		...
		switch(status)
		{
			case 0://正常
				...
				//球击
				if(type===1){
					if (window.LoopInspect===undefined ||(window.LoopInspect && !window.LoopInspect.isGoing) ) {
						this.switchPTZ(true,index);
					}
	...
	*/
	...
	/*********************************************************/
	/**
	 * 这个是绑定事件抽离出来，到使用调用事件的时候只传回调就可以了，不用再做跨浏览器的兼容
	 * @ wangxiaojun
	 * @ 2014-12-5
	 */
	...
	/**
	 * [mouseMoveWindow 鼠标移动事件]
	 * @author Wang Xiaojun
	 * @date   2014-12-05
	 * @param  {Function} fn [回调函数]
	 */
	
	mouseMoveWindow: function(fn) {
		var self = this;
		
		addEvent(self.playerObj, 'MouseMoveWindow', function(index, xPoint, yPoint) {
			fn(index);
		});
	},
	...
	windowClick: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'WndClick', function(index, xPoint, yPoint) {
				fn(index);
		});
	},
	...
	windowDclick: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'WndDClik', function(index, xPoint, yPoint) {
				fn(index);
		});
	},
	...
	switchWindow: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'SwitchWindow', function(index1, index2) {
			// var temp = testData[index1];
			// testData[index1] = testData[index2];
			// testData[index2] = temp;
			fn(index1, index2);
		});
	},
	...
	foucsChange: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'FocusChange', function(oldWindowIndex, newWindowIndex) {
			fn(oldWindowIndex, newWindowIndex);
		});
	},
	...
	layoutChange: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'LayoutChange', function(oldCount, newCount) {
			fn(oldCount, newCount);
		});
	},
	...
	mouseDown: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'MouseDown', function(index, button, x, y) {
	...
	mouseMove: function(fn) {
		...
		var self = this;
		addEvent(self.playerObj, 'MouseMove', function(index, button, x, y) {
			jQuery('body').css('overflow', 'visible');
			fn(index,x,y);
		});
	},
	...
	mouseUp: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'MouseUp', function(index, button, x, y) {
			jQuery('body').css('overflow', 'visible');
				fn(index);
		});
	},
	...
	mouseWheelEvent: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'MouseWheelEvent', function(index, data, x, y) {
			jQuery('body').css('overflow', 'visible');
				fn(index, data);
		});
	},
	...
	mouseLeaveControl: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'MouseLeaveControl', function(index) {
				fn(index);
		});
	},
	...
	fullScreen: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'FullScreen', function() {
	...
	exitFullScreen: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'ExitFullScreen', function() {
			jQuery('body').css('overflow', 'visible');
			fn();
		});
	},
	...
	returnEvet: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'ReturnEvent', function(index, retrunA, returnB) {
			jQuery('body').css('overflow', 'visible');
			fn();
		});
	},
	...
	downLoaePercent: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'DownLoadPercent', function(index, retrun) {
			jQuery('body').css('overflow', 'visible');
			fn(index);
		});
	},
	...
	sizeChanged: function(fn) {
		var self = this;
		addEvent(self.playerObj, 'SizeChanged', function(prewidth, preheight, nowwidth, nowheight) {
			jQuery('body').css('overflow', 'visible');
			fn();
		});
	},

	addOcxEvent:function(name,func){
		var self = this;
		if(this.playerObj.attachEvent){
			this.playerObj.attachEvent("on" + name, func);
		}else{
			this.playerObj.addEventListener(name, func, false);
		}
	},
/******************************************************/
	/**
	...
	**/
	...
	bindEvents: function(){
		...
		var EventList=
		{
			...
			"PlayBackStartOrEnd":"playendback",
			"ExitFullScreen":"exit"
		};
...
/*****************以下是复合接口，供某些页面的特殊调用**************************************************/
	...
	*/
	setFreePath: function(options) {
		...
		if(self.curMaxWinChannel != -1){//存在最大化的窗口
			...
			jQuery(".screenshot-preview .exit").trigger('click');//关闭抓图面板(如果抓图面板存在的话)
	...
	*/
	playHis:function(n,begintime,endtime,vodType,data,fn)
	{
		...
		// begintime =Toolkit.mills2timedata(begintime)+".000";
	...
	*播放历史录像,带回调，传入格式化的参数
	...
	@fn 回调函数
	*/
	playExHis:function(n,begintime,endtime,vodType,data,fn)
	{
		logDict.insertLog('m1','f1','o4','b6',data.name,data.name);
		var str="yyyy-MM-dd hh:mm:ss.000";
		begintime=(new Date(begintime)).format(str);
		...
		//var result = this.playerObj.Play(jsonstr, index);
		 this.playerObj.Play2(jsonstr,index,function(index, result, userParam)
		 {
	...
	*获取实时视频播放的数据格式
	...
	@stype 字符串 类型 取值为real 或 his
	*/
	getPlayData:function(data,stype)
	{
		if(data==-1){return false}

		var getDataIndex=function(data,k)
		{
			var pdata=data[k];
			var obj=
			{
		...
		if(stype=="real")
		{
			...
			{
				return getParam(data.sd_channel);
			}
			...
			{
				return getParam(data.sdChannel);
			}
			else if(data.hdChannel&&data.hdChannel.length>=1)
			{
				return getParam(data.hdChannel);
			}
			else
			{
				return null;
	...
	*/
	PlayerEx:function(str,stype,index,A,fn,timeout)
	{
		...
		var N=this.playerObj.PlayEx(str,index,function(index, result, userParam)
		{
		     ...
		     {
		     	isTimeout=true;
		     	A.push("播放"+stype+"通道连接pvg成功，已经获取到码流！通道信息:"+str);
		     	//这里不执行fn,走到下一个回调执行fn
		     }
		     else
		     {
		...
		setTimeout(function(){
			if(isTimeout)
			{
				A.push("连接"+timeout+"毫秒之后无法连接上，超时");
	...
	*/
	...
	{
		var self=this;
		var sdchannel=camerData.sdchannel;
		var sd_length=sdchannel.length;
		A.push("for循环查找标清通道");
		var EnCoder=null;
		for (var i=0; i <=sd_length-1; i++) 
		{
			var group_id=sdchannel[i].pvg_group_id;
			//1表示编码器，没有录像；2表示DVR  3表示 nvr，高清摄像机的标清码流ipc
			if (group_id == 2 || group_id == 3) 
			{ 
				var stype=(group_id == 2 )?"dvr":"nvr";
				A.push("找到一个"+stype+"通道,尝试播放,通道信息:"+JSON.stringfiy(camerData.sdChannel[i]));
				var str=JSON.stringfiy(sdchannel[i]);
			...
			{
				A.push("找到一个编码器通道,尝试播放,通道信息:"+JSON.stringfiy(camerData.sdChannel[i]));;
				EnCoder=sdchannel[i];
			}
		}
		var str=JSON.stringfiy(EnCoder);
	...
	*/
	...
	PlayRoute:function(camerData,index,fn,timeout)
	{
		...
		var hd_length=camerData.hdchannel.length;
		var sd_length=camerData.sdchannel.length;
		...
		if(hd_length>0)
		{
			var options=camerData.hdchannel[0];
			var str=JSON.stringfiy(options);
	...
	PlayByCameraId:function(cameraid,index,fn,timeout)
	{
		...
		jQuery.ajax({
			...
			success: function(res) 
			{
				if (res.code === 200) 
				{
                   			self.PlayRoute(res.data.cameraInfo,index,fn,timeout);
				} 
	...
	*/
	PlayListTime:function(index,order)
	{
		var self=this;
	...
	window.SelectCamera.ListData[index].input_endTime=input_endTime;
    	...
    	self.playHis(index,beginTime,endTime,vodType,data,function(n)
    	{ 
    		if(n!==0){return}
	...
	/**
	 * [CreateToolBar 创建ocx上的工具条]
	 * @author Wang Xiaojun
	 * @date   2014-11-21
	 * @param  {[object]}   text  [要创建的工具条的对象]
	 * @param  {[type]}   index [预留的参数，但是目前必须传0，后期可能会是窗口的index]
	 * 要创建工具条的的时候必须在初始化ocx的时候调用创建工具条的方法，下面是一个格式
	 * {
		"type": "panel",         	//这个是必须为panel
		"style": "left-bottom",  	//这个是工具条创建成功后显示的位置 可以是left-top, left-bottom, right-top, right-bottom
		"key": 2, 					//每个工具条的key是唯一的，不能重复
		"x": "0", 					//x坐标的大小，int型
		"y": "0",					//y坐标的大小 int型
		"width": "100%",			//工具条的宽
		"height": "32px", 			//工具条的高
		"backColor": "0x242424",	//工具条的背景色
		"transparent": "0.8",
		"animateTime": "1000", 		//毫秒值
		"show":true	,				//工具条是否显示 true显示  false不显示
		//工具条上按钮的集合
		"items": [{
			"type": "button", 		// 三种值：button(按钮),process(播放的进度条),static（静态文本）
			"style": "right-top", 	//按钮显示的位置 可以是left-top, left-bottom ,right-top, right-bottom
			"key": 23,				//按钮的key,也是唯一的，不能一样，包括和工具条的key
			"show": true,			//这个按钮初始化的时候是否显示 true为显示，false为隐藏
			"x": "10",				//按钮的x坐标
			"y": "3",				//按钮的y坐标
			"width": "25px",		//按钮的宽
			"height": "25px",		//按钮的高
			"backColor": "",		//按钮的背景色,一般图片的背景色是透明的，所以背景色要和工具条的背景一致
			"picture": {
				"normal": "4.png",	//按钮正常显示图片
				"hover": "3.png",	//按钮滑过时的图片
				"keydown": "3.png",	//按钮按下时的图片
				"disabled": ""		//按钮不可用是的图片
			},
			"text": "",				//按钮上显示的文字
			"textColor": "",		//按钮上的文字颜色
			"font": "宋体",			//按钮上的文字字体
			"fontSize": 12,			//按钮的字体大小
			"tooltip": "添加预置位",//鼠标放到按钮上的title提示
			//如果有下拉菜单，就可以加上leftOrright,upOrDown,menuList这三个属性，菜单的显示位置是leftOrright,upOrDown属性共同设定的
			"leftOrRight": true,	//按钮菜单的位置	true表示左，false表示右
			"upOrDown": false,		//按钮菜单的位置	true表示上，false表示下
			//菜单的对象
			"menuList": {
				"width": "5px",
				//下拉菜单项的集合
				...
				{
					"text": "数字放大",		//下拉项的文字
					"index": 0,				//index 下拉文字的标识，所有的下拉标识不能重复
					"function": "ElseZoom"	//点击下拉项所触发的方法名
				}, {
					"text": "局部放大",
					"index": 1,
					"function": "CurZoom"
			...
			"function": "addPreset"	//按钮点击是绑定的方法名字
			...
			"funPosChange": "posChange",
			"funRangeChange": "rangeChange"
		}]
	}
	 */
	...
	CreateToolBar: function(text, index) {
		var str = JSON.stringify(text);
		return this.playerObj.CreateToolBar(str, index);

	},


	/**
	 * [ButtonClick 工具条上的按钮的点击事件]
	 * @author Wang Xiaojun
	 * @date   2014-11-21
	 * @param  name [创建工具条的时候每个按钮的邦定的方法名]
	 * @param  {Function} fn   [回调方法]
	 * 这个也是创建工具条的时候一起设定的
	 */


	ButtonClick: function(name, fn) {
		this.playerObj.SetCallBack(name, function(content) {
	...
	/**
	 * [ShowAndEnable 设置按钮是否可以用，或者影藏]
	 * @author Wang Xiaojun
	 * @date   2014-11-21
	 * @param  str   [需要设置按钮的对象]
	 * @param  index [ocx的分屏的index]
	 *  str={
			"key": 25,		//按钮的key
			"show": true,	//是否显示，true为显示，false为不显示
			"enable": true,	//是否可用 ，true为可用，false为不可用
			"status": 0     //这个是预留值
		};
	 * 
	 */
	...
	ShowAndEnable: function(str, index) {
		var strInfo = JSON.stringify(str);
		return this.playerObj.ShowAndEnable(strInfo, index);
	},

	/**
	 * [SetContent 设置文本，只能对静态的设置]
	 * @author Wang Xiaojun
	 * @date   2014-11-21
	 * @param  key   [文本的key值]
	 * @param  str   [要改变的文本]
	...
	SetContent: function(key, str, index) {
		return this.playerObj.SetContent(key, str, index);
	},


	oldStop: function(index) { /*bool:true 暂停播放  false:停止播放*/
		return this.playerObj.Stop(false, index);

	},


	/**
	 * [setFrameFlag 在滑动条上设置标记]
	 * @author Wang Xiaojun
	 * @date   2014-11-22
	 * @param  key       [工具条上滑动条的key值]
	 * @param  startPos [标记开始的位置]
	 * @param  endPos   [标记结束的位置]
	 * @param  bShow     [是否显示] 1表示显示，0表示影藏
	 * @param  index     [窗口的index]
	 *
	 * 这个是在滑动条上设置标记的方法，开始时间和结束时间的单位都是s,bShow的类型是整形
	 */



	setFrameFlag: function(key, startPos, endPos, bShow, index) {
		return this.playerObj.setFrameFlag(key, startPos, endPos, bShow, index);
	},

	/**
	 * [setProgressPos 进度条当前位置指定]
	 * @author Wang Xiaojun
	 * @date   2014-11-22
	 * @param  key   [工具条上滑动条的key值]
	 * @param  pos   [当前要在进度条上的位置]
	 * @param  index [窗口的index]
	 */


	setProgressPos: function(key, pos, index) {
		return this.playerObj.SetProgressPos(key, pos, index);
	},



	/**
	 * [setProgressRange 进度条范围设置]
	 * @author Wang Xiaojun
	 * @date   2014-11-22
	 * @param  key       [工具条上滑动条的key值]
	 * @param  total 	[进度条的总的宽度，ocx就是按进度条的宽度设置成对应的pos值]
	 * @param  index     [窗口的index]
	 */


	setProgressRange: function(key, total, index) {
		return this.playerObj.SetProgressRange(key, total, index);
	},

	/**
	 * [SetProgressTip 鼠标在滑动条上显示]
	 * @author Wang Xiaojun
	 * @date   2014-11-27
	 * @param  key   [工具条上滑动条的key值]
	 * @param  info  [提示的内容]
	...
	SetProgressTip: function(key, info, index) {
		return this.playerObj.SetProgressToolTip(key, info, index);
	},



	/**
	 * [MoveItem 改变某个按钮的一个位置]
	 * @author Wang Xiaojun
	 * @date   2014-11-27
	 * @param  info  [位置改变后的一个json]
	 * @param  index [工具条所在的窗口]
	 */

	/**{
		"style" : "right-top",//按钮的位置
	    "key":11,			//按钮的唯一标识
	    "x": "110",			//x坐标
	    "y": "0",			//y坐标
	*   "width": "25px",	//按钮的宽
	...
	*/
	...
	MoveItem: function(info, index) {
		var strInfo = JSON.stringify(info);
		return this.playerObj.MoveToolBarItem(strInfo, index);
	},


	/**
	 * [resetStatus 工具条上控件状态恢复到初始值]
	 * @author Wang Xiaojun
	 * @date   2014-11-27
	 * @param  key   [工具条上控件的key]
	 * @param  index [工具条所在的窗口]
	 */


	resetStatus: function(key, index) {
		return this.playerObj.ResetStatus(key, index);
	},


	/**
	 * [getStaticContent  获取静态文本的内容]
	 * @author Wang Xiaojun
	 * @date   2014-11-27
	 * @param  key   [工具条上控件的key]
	 * @param  index [工具条所在的窗口]
	...
	getStaticContent: function(key, index) {
		this.playerObj.getStaticContent(key, index);
	},



	/**
	 * [enableToolBar 控制工具条是否显示]
	 * @author Wang Xiaojun
	 * @date   2014-12-01
	 * @param  bool  [是否显示，false是隐藏，true是显示]
	 * @param  index [工具条所在的窗口的index]
	 */


	enableToolBar: function(bool, index) {
		this.playerObj.ShowToolBar(bool, index);
	},


	/**
	 * [downLoadImage 工具条上的按钮图片下载]
	 * @author Wang Xiaojun
	 * @date   2014-12-05
	 * @param  {[type]}   text [url绝对地址]
	 */
	
	downLoadImage: function(text) {
		return this.playerObj.DownLoadImage(text);

	},

	/**
	 * [createJsonMenu 在ocx上创建菜单便于操作]
	 * @author Wang Xiaojun
	 * @date   2014-12-05
	 * @param  {[type]}   str [要创建的菜单是json]
	 * @param  {[type]}   index [鼠标所在ocx窗口的index]
	 */
	...
	createJsonMenu:function(str,index){
		var jsonstr = JSON.stringify(str)
		return this.playerObj.CreateJsonMenu(jsonstr,index);
	},


	/**
	 * [ocxHide ocx在谷歌和火狐下面影藏元素会造成ocx对象销毁。不能达到实际的效果，实际的效果只是想要影藏]
	 * @type {[type]}
	 * @author wangxiaojun
	 * @date 2014-10-15
	 * @param {[type]} ocxid [要影藏的ocx对象的id]
	 */
	

	ocxHide:function(ocxid){
		document.getElementById(ocxid).style.marginLeft = "-9999px";
		document.getElementById(ocxid).width = 1;
		document.getElementById(ocxid).height = 1;
	},
	 ...
	 * @author wangxiaojun
	 * @date 2014-10-15
	 * @param {[type]} ocxid [要显示的ocx对象的id]
	 * @param {[type]} height [要显示的ocx对象的高]
	 * @param {[type]} width [要显示的ocx对象的宽]
	 */
	

	ocxShow:function(ocxid,height,width){
		document.getElementById(ocxid).style.marginLeft = "";
		document.getElementById(ocxid).width = width;
		document.getElementById(ocxid).height = height;
	}
});