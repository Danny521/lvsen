define(['js/cloud-nav.js'],function(cloudNav){
	/*拖拽相关的东西*/
	function Drag(obj){
		var self = this;
	...
	return {
		...
		bindEvents:function(){
			...
			jQuery(window).resize(function(){
				...
				if(jQuery(".pic-wrap").length>0){
					if(SCOPE.context.fileType==="2"||SCOPE.context.fileType==="3"){
						self.picReset();
					}
				}
			}).trigger('resize');
			...
			/*右侧边栏弹出收起*/
			jQuery('.s-bar').on('click',function(e){
				var $this = jQuery(this);
				var leftContent = jQuery('.bg-content');
				var rightContent = jQuery('.bg-sider');
			...
			jQuery(document).on('click', '.tab-title .tab-title-item', function() {
				...
				if($this.closest(".video-title").length>0){
					if(index === $(".video-title .tab-title-outer .video-abstract").attr("data-anchor")){
						$(".content-controll-bar .down-load").hide();
						$(".content-controll-bar .mark-by-hands").hide();
						$(".content-controll-bar .mark-by-auto").hide();
						$(".content-controll-bar .break-line").hide();
						$(".content-controll-bar .derive-nums-info").hide();
					}else if(index === $(".video-title .tab-title-outer .video-multi").attr("data-anchor")){
						$(".content-controll-bar .down-load").hide();
						$(".content-controll-bar .mark-by-hands").hide();
						$(".content-controll-bar .mark-by-auto").hide();
						$(".content-controll-bar .break-line").hide();
						...
						$(".content-controll-bar .down-load").show();
						$(".content-controll-bar .mark-by-hands").show();
						$(".content-controll-bar .mark-by-auto").show();
						$(".content-controll-bar .break-line").show();
						$(".content-controll-bar .derive-nums-info").show();
					}
				}
				jQuery('.tab-content').find('[data-target=' + index + ']').addClass('active').siblings().removeClass('active');
				jQuery(window).trigger('resize');
			});
			jQuery(".close-eyes").click(function(){
				$(".eyes").hide();
			});
			/*1:1*/
			 ...
			 */
			jQuery(document).on("mousedown",".left-bar",function(){
				var $this = $(this);
				$this.addClass("down");
			});
			jQuery(document).on("mousedown",".right-bar",function(){
				var $this = $(this);
				$this.addClass("down");
			});
			jQuery(document).on("mouseup",".left-bar",function(){
				var $this = $(this);
				$this.removeClass("down");
			});
			jQuery(document).on("mouseup",".right-bar",function(){
				var $this = $(this);
				$this.removeClass("down");
			});
		},
		...
		eyeMouseMoveCallback:function(size){
			/*position是eye小窗口的拖动变化量*/
			/*var pic = jQuery('.big-pic');
			var curPicWidth = pic.width();position,parDom
			var curPicHeight = pic.height();
			var $outline = jQuery(".outline")[0];
			var $eyes = jQuery(".eyes");
			if(position.x >= 0){
				var changeX = -position.x / parDom.width() * curPicWidth + 200;
				pic.offset({left:changeX});
			}

			if(position.y >= 0){
				var changeY = -position.y / parDom.height() * curPicHeight + 280;
				pic.offset({top:changeY});
			}*/
			$('.big-pic').offset({
		...
		picReset:function(src){
			var self = this,
				src =  src || SCOPE.context.picture || SCOPE.context.filePath,
				img = new Image(),
