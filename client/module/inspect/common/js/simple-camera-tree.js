define(['jquery','mootools'],function(){
    var DefenseCamera = new Class({
        ...
        options: {
            ...
            templateUrl: "/module/inspect/common/inc/simple-camera-tree.html",
        ...
        addHelper: function () {
            ...
            Handlebars.registerHelper("groupStatus", function (sharedStatus) {
             ...
             Handlebars.registerHelper('hasplayBtn', function(flag,type) {
                if(type==="group"){
                    return "hidden";
                }else{
                   if(flag){
            ...
            if (context.options.selectable) {
                if (parentNode.children("i.checkbox").is("i.selected") || ((init || key) && context.options.defaultSelected)) {
                    parentNode.append(context.render({
                        "cameras": receiveData,
                        "treeNeedSelectBox": context.options.treeNeedSelectBox,
                        "level": level,
                        "init": init,
                        "selected": "selected",
                        "selectable": context.options.selectable,
                        "size": receiveData.length,
                        "hasPlayBtn":context.options.hasPlayBtn
                    }));
                } else {
                    parentNode.append(context.render({
                        "cameras": receiveData,
                ...
                success: function (res) {
                    ...
                    if (getRootFlag) {
                        if (res && res.code === 200 && res.data.cameras) {
                            ...
                            /*开始：addby wujingwen*/
                            setTimeout(function () {
                                jQuery(".simple-camera-tree-panel>ul>li>i.fold").trigger("click");
                            }, 10);
                            /*结束*/
                        } else {
                            parentNode.attr("data-loaded", 1);
                        ...
                        if (res && res.code === 200 && res.data.cameras.length > 0) {
                            ...
                            //    self.appendHTML(receiveData.splice(0, 49), parentNode, self, initFlag, params.key);
                                self.appendHTML(receiveData, parentNode, self, initFlag, params.key);
