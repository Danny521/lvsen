	...
	DragItems.prototype = {
		...
		bindEvent: function () {
			...
			self.ctrlInfo.bindObject.find("li").each(function () {
				...
				//鼠标提起事件
				event.addEvent(thisObj, "mouseup", function () {
		...
		onItemsMouseDown: function (evt, obj) {
			...
			if (self.isInItemRect(mouseX, mouseY)) {
				...
				self.topDis = mouseY - jQuery(obj).position().top;
				//浮动显示待移动元素
		...
		onItemsMouseMove: function (evt) {
			...
			if (self.isMouseDown) {
				...
				var mouseX = self.getMousePos(evt).x, mouseY = self.getMousePos(evt).y, cssInfo = {};
				//允许在x、y轴上进行拖动
