define([
	// 左侧摄像机树
	'/module/settings/inspectPlanSet/js/camera-tree.js',
	// 全局变量
	'/module/settings/inspectPlanSet/js/globar-varibal.js',
	// 布防任务设置model层
	'/module/settings/inspectPlanSet/js/model/inspectModel.js',

	'/module/settings/inspectPlanSet/js/controller/inspectController.js',

	'jquery.watch'
], function(inspectCameraTree, globalVar, model , inspectCtr) {
	return {
		options: {
			// 摄像机树对象
			cameraTree: null,
		...
		init: function() {
			...
			//读取接口加载加载摄像机树
			options.cameraTree = new inspectCameraTree({
				"node": ".rule-camera-tree #cameraTree",
			...
			//搜索
			jQuery(".midLeft input[name='taskName']").watch({
				wait: 200,
				captureLength: 0,
			...
			jQuery(".midLeft .searchbox button").off("click").on("click", function() {
				var value = jQuery(".midLeft input[name='taskName']").val();
				//触发查询
				options.cameraTree.search({
					queryKey: value
				});
		...
		treeItemClick: function (el) {
			var 
				self = this, 
				id   = globalVar.cameraData ? globalVar.cameraData.id : "",
				$li  = el.closest("li"),
				cameraData = $li.data();
				// 给当前元素赋予新的值
