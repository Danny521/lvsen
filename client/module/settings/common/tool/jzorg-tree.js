...
define(["ajaxModel","base.self",'jquery.watch'], function(ajaxModel){

	var Tree = new Class({
		Implements: [Events, Options],
		options: {
			url:"/service/org/get_child_orgs_by_parent?"+ window.sysConfig.getOrgMode(),	//根据父组织获取子组织
			rootUrl:"/service/org/get_root_org?org_type=1",			//获取根组织
			searchUrl:"/service/org/get_child_orgs_by_name",	//根据组织名搜索
			templateUrl:"/module/settings/common/tool/jzsmall-tree.html",
		...
		updateLiSpan:function (type,params) {
			...
			if(type === "delete"){
				...
				if(parent.length > 0){
					...
					parent = jQuery(this.options.node).find("li[data-id="+params.parentId+"]");
					// 如果找到的父节点 就刷新
					if(parent.length > 0){
						if(parent && parent.attr("data-loaded")){
							parent.children("ul").remove();
							parent.removeAttr("data-loaded");
							parent.removeClass("active");
							parent.children("i.fold").click();
				...
				var p = jQuery(this.options.node).find("li[data-id=" + params.parentId + "]");
				if (p && p.attr("data-loaded")) {
					p.children("ul").remove();
					p.removeAttr("data-loaded");
					p.removeClass("active");
					p.children("i.fold").click();
				}
		...
		*/
		...
		bindEvent:function(parentNode,initFlag){
			...
			if(self.options.selectable){
				...
				parentNode.find("li>i.radio-leaf").click(function(){
					var tem = jQuery(this);
		 ...
		 */
		updateClickEffect:function(id){
			if (!id) {
				return;
			}
			var self = this,
				el = jQuery(self.options.node).find("li[data-id=" + id + "]");
			if(el.length > 0){
				self.addClickEffect(el.children("span"));
			}
		},
		/*
		 *	添加点击样式
