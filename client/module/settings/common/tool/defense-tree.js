...
define(["ajaxModel","base.self"], function(ajaxModel){
	...
	var DefenseTree = new Class({
		...
		options: {
			...
			templateUrl:"/inc/settings/DefenseTree.html",
		...
		getOrgPathList:function(currentOrgId){
			var self = this;
			ajaxModel.getData(self.options.getParentsUrl,{orgId: currentOrgId},{async:false}).then(function (res) {
					if(res.code === 200){
		...
		loadTemplate:function() {
			var self  = this;
			ajaxModel.getData(self.options.templateUrl).then(function(tmp){
					...
					if(getRootFlag){
						...
						if(res && res.code === 200 && res.data.cameras.length > 0){
							...
							if(params.key){
								 ...
								 */
								parentNode.attr("data-loaded",1);
								parentNode.append("<ul><li><div class='no-data'></div></li></ul>");
							}
								self.updateScrollBar();
