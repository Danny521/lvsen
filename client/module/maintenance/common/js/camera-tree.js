 ...
 * 资源(摄像机)树
 */
define(["pubsub","jquery","base.self","mootools","handlebars"],function(PubSub){
    var CameraTree = new Class({
        ...
        options: {
            ...
            rootUrl:"/service/resource/get_root_camera?isRoot="+ window.sysConfig.getResMode(),
        ...
        addHelper:function(){
            ...
            Handlebars.registerHelper('isHD', function(isHD,options) {
                if(isHD === 1){
                    return "hasHD";
                }
            });
        ...
        bindEvent:function(parentNode,initFlag){
            ...
            //parentNode.on("click","ul li i.fold")
            // + 点击事件
            //parentNode.find(node).closest("li").children("i.fold").on("click",
            parentNode.on("click","ul li i.fold",function(){
        ...
        filterData : function(data,type){
            return data;
            var treeId, camera, treeLen, camLen, dataLen,i=0, j= 0,returnData = [];

            //mintenance.data.taskData = false;
            if(mintenance.data.taskData && data){
                treeId  = mintenance.data.taskData.treeId;  // 任务中的组织
                camera  = mintenance.data.taskData.camera;  // 任务中的摄像机
                treeLen = treeId.length;                    // 任务中的组织的长度
                camLen  = camera.length;                    // 任务中的摄像机的长度
                dataLen = data.length;                      // 被对比的数据的长度

                if(type === "org"){
                    for(i=0;i<treeLen;i++){
                        for(j=0;j<dataLen;j++){
                            if(((data[j].strOrgId || data[j].id).toString()) === (treeId[i].toString())){
                ...
                if(type === "camera"){
                    for(i=0;i<camLen;i++){
                        for(j=0;j<dataLen;j++){
                            if((data[j].id-0) === (camera[i].cameraId-0)){
                                returnData.push(data[j]);
                            }
                        }
                    }
        ...
        appendHTML:function(receiveData,receiveData2,parentNode,context,init){
            ...
            if(selectable){
                if(parentNode.children("i.checkbox").is("i.selected")){
                    html = context.render({"orgs":self.filterData(receiveData,"org"),"level":level,"cameras":self.filterData(receiveData2,"camera"),"init":init,"selected":"selected","selectable":selectable,"size":receiveData.length});
                }else{
                    html = context.render({"orgs":self.filterData(receiveData,"org"),"level":level,"cameras":self.filterData(receiveData2,"camera"),"init":init,"selected":"","selectable":selectable,"size":receiveData.length});
                }
            }else{
                html = context.render({"orgs":self.filterData(receiveData,"org"),"level":level,"cameras":self.filterData(receiveData2,"camera"),"init":init,"selectable":selectable,"size":receiveData.length});

            }

            parentNode.append($("<ul class='camera-area-list'></ul>"));

            ul = parentNode.find("ul.camera-area-list");

            lis = html.replace("<ul>","").replace("</ul>","").replace(/li>/g,"li>||").replace(/\r\n/g,"").split("||");

            lis_len = lis.length;

            len = Math.ceil(lis_len / per);
            //console.log("Render End Time : " + (new Date()).getTime());


            // 优化逻辑，如果 是小于需优化的长度 的话，直接插入，不用延迟优化
            if(lis_len<per){
                ul.append(lis.join(""));
                ...
                setTimeout(function(){
                    if(index>=len){
                        if(options.selectable){
                            self.selectEl();
                            if(parentNode.attr("data-default")){
                                self.walkDown(parentNode.children("i.checkbox"));
                            }
                        }
                    ...
                    var htm = lis.slice(index*per,((index+1)*per)).join("");
                    htm && ul.append(htm);
                    index++;
                    _setTimeout();
                },200);
