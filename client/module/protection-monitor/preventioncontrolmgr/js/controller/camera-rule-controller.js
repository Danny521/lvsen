...
define([
	...
	'js/controller/camera-rule-video-controller',
	'DrawEditor',
	'base.self'
	...
	var CameraRuleSetting = new new Class({
		...
		bindEvents: function() {
			...
			jQuery(document).on("click", ".control-video .alarm-events-content-video .content-top-video-tool .intercept", function(e) {
				e.stopPropagation();
				self.dealScreenShotEvent(this);
			});
			//布控任务设置框线规则时，播放窗口右上角眼睛的点击事件
			jQuery(document).on("click", ".control-video .alarm-events-content-video .content-top-video-tool .see", function(e) {
				e.stopPropagation();
		...
		screenShotAndShowLines: function() {
			//判断是否已经有画布，没有则提示用户先暂停（抓图）
			if (jQuery("#TempSnapPicture").length === 0) {
				//直接抓图
				jQuery(".control-video .alarm-events-content-video .content-top-video-tool .intercept").trigger("click");
			}
		},
		...
		dealScreenShotEvent: function(obj) {
			...
			if (jQuery(obj).hasClass("icon_play")) {
				...
				jQuery("#TempSnapPicture").attr({
					...
					document.getElementById("UIOCXCONTROL").style.marginLeft = "-5px";
