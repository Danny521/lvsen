...
define(['ajaxModel'],function(ajaxModel){
	var Model = {
		...
		//调用获取摄像机列表接口
		GET_CAMERA_LIST: '/service/defence/cameras',
		//获取布防摄像机布防任务列表
		 ...
		 */
		//前往视图库接口
		RELOAD_INTO_VIEW:'/service/pvd/realtime/structure',
		// 一键开启或者关闭所有任务
		TOGGLE_ALL_TASK_STATUS: "/service/defence/task",
		/**
		 * 	设置任务数限制,渲染树数据，只针对任务数设置模块
		 */
		SET_TASK_NUM:'/service/defence/limits'
		...
		ajaxEvents: function(){
			 ...
			 */
				...
				checkSingleTask:function(data,success,error){
					ajaxModel.getData(self.URLS.SINGLE_CONTROL_TASK+'/view/'+data.id,data).then(success,error);
				},
				//保存布控任务
				...
				//实时化结构
				moveIntoView:function(data,success,error){
					ajaxModel.getData(self.URLS.RELOAD_INTO_VIEW,data).then(success,error);
				},
				// 一键开启或者关闭全部任务
				...
				//添加布防路数限制
				setTaskNumSet:function(data,success,error){
					ajaxModel.postData(self.URLS.SET_TASK_NUM,data).then(success,error);
				}
			};
		}
	};
