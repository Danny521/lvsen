...
define([
...
], function(linkageControl, linkageModel, timeTemplate, linkageCtrl) {
	...
	LinkageView.prototype = {
		...
		bindEvent: function() {
			var self = this;
			jQuery("#control-main-content").off("click", ".linkage-check:not(.disabled)")
			.on("click", ".linkage-check:not(.disabled)", function() {
				...
				// 获取布控任务id
				var taskId = linkageControl.taskId;
				//布控联动设置 by zhangxinyu
				linkageCtrl.init("ctrl", taskId);
				document.getElementById("npgis") && (document.getElementById("npgis").style.visibility = "hidden");
				document.getElementById("UIOCXCONTROL") && (document.getElementById("UIOCXCONTROL").style.visibility = "hidden");
				return false;
			});
		},
		/**
			...
			self.getTemplate("linkagePanelTemp", function(temp) {
				self.dialogObj = new CommonDialog({
					...
					prehide: function() {
						...
						document.getElementById("UIOCXCONTROL") && (document.getElementById("UIOCXCONTROL").style.visibility = "visible");
					}
				});
				// 按钮取消禁用
