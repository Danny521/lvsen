...
], function(pvaMapController, model, mapModel, helper) {
	return {
		options: {
			templateMap: {},
			ocxDom: ""
		...
		initMap: function() {
			...
			// 显示资源列表dom
			jQuery("#defence-setting-mapId").find(".defence-map-resource-layers").show();
		},
		initMapToolBar: function() {
			var self = this,
				template = self.options.templateMap.pvaMapTemplate;
			//加载左侧地图名称
			jQuery('#defence-setting-gismap').find(".map-region").html(template({ mapRegion: true }));
			//加载右侧工具栏
			jQuery('#defence-setting-gismap').find(".map-operator").html(template({
				tools: {
					// 地图放大
					max: true,
					// 地图缩小
			...
			//加载卫星图层
			jQuery("#defence-setting-mapId").find(".opera-container").html(template({
				//图层切换
				switchLayer: false
			}));
		},
		bindEvents: function() {
			...
			//屏蔽地图右键
			jQuery("#defence-setting-mapId > div.olMapViewport").bind('contextmenu', function () {
				return false;
			});
			//地图右键取消操作
			jQuery("#defence-setting-mapId > div.olMapViewport").bind("mousedown", function (e) {
				//右键
				if (3 === e.which) {
			...
			// 地图放大
			jQuery("#defence-main-content").on("click", "#map-tool-right .map-tools-list .map-max", function () {
				mapManage.zoomIn();
				//激活鼠标文字跟踪
			...
			// 地图缩小
			jQuery("#defence-main-content").on("click", "#map-tool-right .map-tools-list .map-min", function () {
				mapManage.zoomOut();
			...
			//测距
			jQuery("#defence-main-content").on("click", "#map-tool-right .map-tools-list .map-measure", function () {
				pvaMapController.options.measuretool.setMode(NPMapLib.MEASURE_MODE_DISTANCE);
			});
			//清除
			jQuery("#defence-main-content").on("click", "#map-tool-right .map-tools-list .map-clear", function () {
				try {
			...
			//全屏
			jQuery("#defence-main-content").on("click", "#map-tool-right .map-fullscreen", function () {
				pvaMapController.options.isFullscreen = true;
				self.fullscreen(this, template);
			});
			//退出全屏
			jQuery("#defence-main-content").on("click", "#map-tool-right .map-exitfullscreen", function () {
			...
			//切换图层
			jQuery("#defence-main-content").on("click", ".opera-container a.map-layer-normal,.opera-container a.map-layer-satellite", function () {
		...
		fullscreen: function (obj, template) {
			...
			jQuery("#defence-setting-sidebar").hide();
			jQuery("#content .wrapper").css("top", "0px");
			jQuery("#defence-setting-major").css({
				top: "0px",
				left: "0px"
			});
			//加载全屏模式下的工具条
		...
		exitFullscreen: function (obj, template) {
			var This = jQuery(obj);
			//显示地图外的其他元素
			jQuery("#navigator,#header,#defence-setting-sidebar").show();
			jQuery("#content .wrapper").css("top", "86px");
			jQuery("#defence-setting-major").css({
				top: "96px",
				left: jQuery("#defence-setting-sidebar").width()
			});
			//加载非全屏状态下的工具条
			This.parent().empty().html(template({
				fullscreen: true
		...
		bindResourceEvents: function(){
			//资源图层切换事件绑定
			jQuery(".defence-map-resource-layers").hover(function () {
			...
			//资源图层控制
			jQuery("#defence-main-content").on("click", ".defence-map-resource-layers ul li", function () {
				var This = jQuery(this),
					resourceMap = {
		 ...
		 * 在地图上显示摄像机信息
		 **/
		showMapCameraInfo: function (cameraId) {
			var self = this,
				// 模板
				template = self.options.templateMap.pvaMapTemplate,
				// 摄像机数据
			...
			// 左侧树选中当前摄像机
			var cameraTreeView = require("/module/protection-monitor/defencesetting/js/view/defence/first-camera-tree-view.js");
			cameraTreeView.checkCameraById(cameraId);
			//触发加载信息窗口
		...
		bindInfoWinEvent: function() {
			...
			//查看实时视频
			jQuery(".map-camera-info .camera-status-online").off("click").on("click", function () {
				//摄像机信息窗上查看实时视频
